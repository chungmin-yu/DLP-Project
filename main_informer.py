...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--scale", type=bool, default=False, help="scale the dataset (Add by Stock)"
    )
    parser.add_argument(
        "--freq",
        type=str,
    ...
    parser.add_argument(
        "--d_layers", type=int, default=5, help="num of decoder layers"
    )  ## use 12
    ...
    parser.add_argument(
        ...
        type=str,
        default="prob",
        help="attention used in encoder, options:[prob, log, full]",
    )
    parser.add_argument(
        "--embed",
        type=str,
    ...
    parser.add_argument("--lradj", type=str, default="1", help="adjust learning rate")
    ...
    for ii in range(args.itr):
        ...
        if CONTINUE_TRAIN:
            exp.model.load_state_dict(
                torch.load(f"results/{MODEL_DIRECTORY}/checkpoint.pth")
            )
            exp.model.train()
